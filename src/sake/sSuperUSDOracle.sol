...
contract sSuperUSDOracle is IsSuperUSDOracle {
    ...
    function setMaxRate(uint256 _maxRate) external onlyOwner {
        // New max rate must be greater than MIN_RATE
    ...
    function setMinRate(uint256 _minRate) external onlyOwner {
        // New min rate must be less than MAX_RATE
    ...
    view
    returns (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound) {
        IAccountant accountant = IAccountant(sSuperUSDOracleAddress);
        ...
        return (0, int256(adjustedRate), 0, 0, 0);
    }
}
